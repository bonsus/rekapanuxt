...
import type { SkuSearchResult, OrderItemPayload } from '~/types'
...
const { createOrder, loading, searchSkus } = useOrders()
...
async function handleSubmit() {
  ...
  const hasEmpty = items.value.some(i => !i.productName.trim())
  if (hasEmpty) { error.value = 'Nama produk item wajib diisi'; return }
  ...
  try {
    ...
    error.value = (e as { data?: { message?: string } })?.data?.message ?? 'Gagal membuat order'
  }
}
  ...
  <div class="max-w-4xl mx-auto space-y-6 pb-10">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input v-model="form.orderNumber" type="text" placeholder="INV-001" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        <div>
        ...
        <div v-if="form.status === 'CANCELLED'">
          ...
          <select v-model="form.cancelBy" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
            <option value="">-</option>
            <option v-for="c in cancelByOptions.slice(1)" :key="c" :value="c">{{ c }}</option>
          </select>
        </div>
        <div v-if="form.status === 'CANCELLED'" class="sm:col-span-2">
          <label class="block text-xs text-gray-500 mb-1">Alasan Pembatalan</label>
          <input v-model="form.cancelReason" type="text" placeholder="Alasan..." class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-4">
      <div class="flex items-center justify-between">
        ...
        <button
          ...
          @click="() => { addEmptyItem(); initSkuSearch(items.length - 1) }"
        >+ Tambah Item</button>
      </div>
      ...
      <div v-for="(item, idx) in items" :key="idx" class="border border-gray-100 rounded-xl p-4 space-y-3 bg-gray-50/50">
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 relative">
            <label class="block text-xs text-gray-500 mb-1">Produk / SKU <span class="text-red-500">*</span></label>
            <input
        ...
        <!-- Manual fields if no SKU selected -->
        <div v-if="!item.skuId" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            ...
            <input v-model="item.productName" type="text" placeholder="Nama produk" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
          </div>
          <div>
            <label class="block text-xs text-gray-500 mb-1">Kategori</label>
            <input v-model="item.productCategory" type="text" placeholder="Kategori" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
          </div>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-4">
      <div class="flex items-center gap-3">
        ...
        <label class="flex items-center gap-2 text-sm text-gray-500 cursor-pointer">
          <input v-model="customerForm.enabled" type="checkbox" class="rounded">
          Isi data pelanggan
        </label>
      </div>
      <div v-if="customerForm.enabled" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label class="block text-xs text-gray-500 mb-1">Customer ID</label>
          <input v-model="customerForm.customerId" type="text" placeholder="ID pelanggan (opsional)" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        <div>
          <label class="block text-xs text-gray-500 mb-1">Nama <span class="text-red-500">*</span></label>
          <input v-model="customerForm.name" type="text" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        <div>
          <label class="block text-xs text-gray-500 mb-1">No. Telepon</label>
          <input v-model="customerForm.phone" type="text" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        <div class="sm:col-span-2">
          <label class="block text-xs text-gray-500 mb-1">Alamat Detail</label>
          <input v-model="customerForm.address" type="text" placeholder="Nama jalan, nomor rumah, dll" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        <div>
          <label class="block text-xs text-gray-500 mb-1">Provinsi</label>
          <input v-model="customerForm.province" type="text" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        <div>
          <label class="block text-xs text-gray-500 mb-1">Kota</label>
          <input v-model="customerForm.city" type="text" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        <div>
          <label class="block text-xs text-gray-500 mb-1">Kecamatan</label>
          <input v-model="customerForm.district" type="text" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        <div>
          <label class="block text-xs text-gray-500 mb-1">Kode Pos</label>
          <input v-model="customerForm.zipcode" type="text" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5 space-y-4">
      <div class="flex items-center gap-3">
        ...
        <label class="flex items-center gap-2 text-sm text-gray-500 cursor-pointer">
          <input v-model="shippingForm.enabled" type="checkbox" class="rounded">
          Isi data pengiriman
        </label>
      </div>
      <div v-if="shippingForm.enabled" class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div>
          ...
          <input v-model="shippingForm.name" type="text" placeholder="JNE, TIKI, dll" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        <div>
          <label class="block text-xs text-gray-500 mb-1">Layanan</label>
          <input v-model="shippingForm.serviceName" type="text" placeholder="REG, YES, dll" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        <div>
          <label class="block text-xs text-gray-500 mb-1">No. Resi</label>
          <input v-model="shippingForm.trackingNumber" type="text" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
      </div>
    </div>
